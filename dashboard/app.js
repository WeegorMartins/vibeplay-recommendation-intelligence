...
function render(data){
  ...
  Plotly.newPlot('funnelChart',[{labels:data.event_funnel.map(x=>x.event),values:data.event_funnel.map(x=>x.value),type:'donut',hole:.72,marker:{colors:['#46f09b','#23b779','#9cffe0','#48655c']},textinfo:'percent',hovertemplate:'%{label}: %{value:,}<extra></extra>'}],layout({showlegend:true,legend:{orientation:'h',y:-.08},annotations:[{text:'eventos',showarrow:false,font:{size:12,color:colors.muted}}]}),plotConfig);
